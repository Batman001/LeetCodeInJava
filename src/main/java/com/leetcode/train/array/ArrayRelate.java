...
public class ArrayRelate {
    ...
    private int findUnsortedSubarray(int[] nums){
        int n = nums.length;
        int beg = -1;
        // end is -2 is because it works if the com.leetcode.train.array is already in ascending order
        int end = -2;
        // from right to left
        int min = nums[n-1];
        // from left to right
        ...
        // if com.leetcode.train.array is already in ascending order, -2 - (-1) + 1 = 0
        return end - beg + 1;
    }
